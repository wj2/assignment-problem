...
bf = "/Users/wjj/Dropbox/research/analysis/" "assignment/figs/"
colors = gps.nms_colors
n_colors = gps.assignment_num_colors
r_colors = np.array([[186, 143, 149], [167, 148, 183], [214, 142, 214]]) / 255
...
def figure1(basefolder=bf, gen_panels=None, data=None):
    ...
    if "b" in gen_panels:
        ...
        plot_eg_overlap(ol_ax_1, ol_ax_2, color1=r_colors[0], color2=r_colors[1])

    stim_locs = (20, 40, 80)
    dx = 10**2
    dy = 10**2
    noise_delts = np.array([[6, 4, -5], [12, -21, 3]])
    if "c" in gen_panels:
        ...
        plot_noise_eg(
            ...
            dx,
            dy,
            (noise3_ax, noise2_ax, noise1_ax),
            noise_delts=noise_delts,
            r_c1=r_colors[0],
            r_c2=r_colors[1],
        )
...
def figure2(basefolder=bf, gen_panels=None, data=None):
    ...
    ol_dims = (1, 2, 3)
    ol_funcs = [am.line_picking_line, am.line_picking_square, am.line_picking_cube]
    if "b" in gen_panels:
        distrib_ax = f.add_subplot(distance_distrib_grid)
        plot_distance_distrib_prob(distrib_ax, overlap_dims=ol_dims, funcs=ol_funcs)

    ds = (10,)
    size = 100
    bound_color = np.array((0.3, 0.3, 0.3))
    if "c" in gen_panels:
        ep_ax = f.add_subplot(distance_prob_grid)
        ep_ax.set_yscale("log")
        out = plot_distance_error_prob(
            ep_ax,
            distortions=ds,
            color=bound_color,
        ...
        curve, dists = out
        plot_distance_dim_means(
            ep_ax, ol_dims, ol_funcs, size, curve[0], dists, colors=colors
        )
    ...
    if "de" in data.keys():
        ...
        aes_est = estimate_empirical_aes(
            ol_dims, stim_count[0], pr, size, n_ests=n_ests
        )
    ...
    if "de" in gen_panels:
        ...
        plot_ae_error(
            ae_ax, red_ax, aes[:, 0], redund[:, 0], ol_dims, pr, aes_est=aes_est
        )

    fname = os.path.join(bf, "fig2-py.svg")
    f.savefig(fname, bbox_inches="tight", transparent=True)
    fname = os.path.join(bf, "fig2-py.pdf")
    f.savefig(fname, bbox_inches="tight", transparent=True)
    return data
...
def figure3(basefolder=bf, gen_panels=None, data=None):
    ...
    delta_x = np.linspace(0, 0.9, 1000)
    ds = 10
    ...
    if "c" in gen_panels:
        ...
        gpl.plot_trace_werr(delta_x, dx, ax=dd_ax, label=r"$D_{X}$", color=r_colors[0])
        gpl.plot_trace_werr(
            delta_x, dy, ax=dd_ax, label=r"$D_{Y}$", log_y=True, color=r_colors[1]
        )
        xl = dd_ax.get_xlim()
    ...
    if "d" in gen_panels:
        ...
        plot_distance_error_prob(
            dae_ax, distortions=distort, label_num=delts, color=n_colors[0]
        )

    size = 100
    stim_count = (2,)
    pr = np.logspace(0, 4, 50)
    delta_d = np.array([0, 0.5, 0.9])
    ol_dims = (1,)
    n_ests = 5000
    if "ef" in data.keys():
        ...
        aes_est = estimate_empirical_aes(
            ol_dims, stim_count[0], pr, size, delta_d=delta_d, n_ests=n_ests
        )
        aes, redund = compute_ae_red(ol_dims, stim_count, pr, size, delta_d=delta_d)
    ...
    if "ef" in gen_panels:
        ...
        plot_ae_error(
            ae_ax, red_ax, aes, redund, ol_dims, pr, delta_d=delta_d, aes_est=aes_est
        )
...
def figure4(basefolder=bf, gen_panels=None, data=None):
    ...
    bits = 20
    delta_d = 0.5
    unique_col = (0.4, 0.1, 0.1)
    common_col = (0.1, 0.4, 0.4)
    if "bc" in gen_panels:
        ub, cbx, cby, ds, dx, dy = compute_bit_alloc(bits, k, c, delta_d, size)
        unique_heights = (ub,) * (k - c)
        common_heights = (cbx, cby)
        unique_labels = tuple(r"$f_" + "{}$".format(i + 1) for i in range(k - c))
        c_labels = (
            (r"$f^{X}_" + "{}$".format(i + 1), r"$f^{Y}_" + "{}$".format(i + 1))
        ...
        alloc_ax.bar(range(k - c, k + c), common_heights, color=common_col)
...
def figure5(basefolder=bf, gen_panels=None, data=None):
    ...
    if "abcde" in gen_panels:
        ...
        ld_ax = f.add_subplot(local_delt_grid, sharex=aed_ax, sharey=lc_ax)
        ...
        for i, c in enumerate(c_xys):
            gpl.plot_trace_werr(
                bits, aes[i, delt_ind], ax=aec_ax, label="C = {}".format(c), log_y=True
            )
            gpl.plot_trace_werr(bits, evs[i, delt_ind], ax=lc_ax, log_y=True)
            gpl.plot_trace_werr(
                delts, aes[i, :, bits_ind], ax=aed_ax, linestyle="dashed", log_y=True
            )
            gpl.plot_trace_werr(
                delts, evs[i, :, bits_ind], ax=ld_ax, linestyle="dashed"
            )
            ...
            l = gpl.plot_trace_werr(
                aes[i, delt_ind], evs[i, delt_ind], ax=lae_ax, log_y=True, log_x=True
            )
            ...
            gpl.plot_trace_werr(
                aes[i, :, -1], evs[i, :, -1], ax=lae_i, log_x=True, log_y=True
            )
            if c == max(c_xys):
                gpl.label_plot_line(
                    aes[i, delt_ind], evs[i, delt_ind], r"$\leftarrow$ info", ax=lae_ax
                )
                gpl.label_plot_line(
                    aes[i, :, 0][::-1],
                    evs[i, :, 0][::-1],
...
def _plot_rfs(
    ...
    if make_scales:
        ...
        gpl.make_yaxis_scale_bar(
            ax, 1, label="dimension 2", double=False, text_buff=0.2
        )
        gpl.clean_plot(ax, 0)


def plot_transition(
    total_dims, total_pwrs, total_units, nrs=(1, 2, 3), overlaps=(0, 1, 2)
):
    ov_dict = {
        ov: am.compute_transition(
            total_dims, total_pwrs, total_units, nrs=nrs, overlap=ov
        )
        for ov in overlaps
    }
    return ov_dict
...
def figure_fi_intro(basefolder=bf, gen_panels=None, data=None):
    ...
    if data.get("a") is None and "a" in gen_panels:
        ...
        rf_cents, rf_wids = rfm.get_random_uniform_fill(n_units, rf_distrs, wid=w)
    ...
    thr_col = np.array((148, 28, 47)) / 255 + 0.2
    ...
    if "wid" in gen_panels:
        ...
        wid_ax.plot(
            wid, (1 - nl_prob) * l_mse, color=local_col, alpha=comp_alpha, label="local"
        )
        wid_ax.plot(
            wid, nl_prob * nl_mse, color=thr_col, alpha=comp_alpha, label="threshold"
        )
    ...
    if "bcd" in gen_panels:
        ...
        hist_ax.hist(
            errs[errs > 10 ** (-1)], bins=bins, label="threshold", color=thr_col
        )
    ...
    if data.get("red") is None and "red" in gen_panels:
        ...
        for i, c in enumerate(cs):
            ...
            mse1 = np.array(
                list(
                    rfm.min_mse_vec(pwr_i, n_units, k1, ret_components=True)[1]
                    for pwr_i in pwrs
                )
            )
            mse2 = np.array(
                list(
                    rfm.min_mse_vec(pwr_i, n_units, k2, ret_components=True)[1]
                    for pwr_i in pwrs
                )
            )
            ...
            ae_r = np.array(
                list(
                    am.integrate_assignment_error(
                        (2,), mse1[i : i + 1], mse2[i : i + 1], c, p=1
                    )
    ...
    if "red" in gen_panels:
        ...
        for i, c in enumerate(cs):
            ...
            if i == 0:
                ...
                mse_conn = np.concatenate(
                    (mse_conn, np.expand_dims(mse_avg, 0)), axis=0
                )
                ae_conn = np.concatenate((ae_conn, np.expand_dims(ae_r, 0)), axis=0)
                red_conn = np.concatenate((red_conn, np.expand_dims(red, 0)), axis=0)

            gpl.plot_trace_werr(red, mse_avg, ax=red_mse_ax, log_y=True)
            gpl.plot_trace_werr(
                red, ae_r, ax=red_ae_ax, log_y=True, label="C = {}".format(c)
            )
        lw = 0.8
        l_col = (0.8, 0.8, 0.8)
        red_mse_ax.plot(
            red_conn[:, ::2], mse_conn[:, ::2], color=l_col, zorder=-1, linewidth=lw
        )
        red_ae_ax.plot(
            red_conn[:, (0, -1)],
            ae_conn[:, (0, -1)],
...
def figure_fi(basefolder=bf, gen_panels=None, data=None):
    ...
    if data.get("bc") is None and "bc" in gen_panels:
        ...
        for i, ov in enumerate(overlaps):
            ...
            for j, nu in enumerate(n_units):
                ...
                fi, pwr, distortion = am.compute_fi(
                    rf_cents, use_wids, pwr_def, stim_distr
                )
                fi_emp_nu[i, j] = fi
            ...
            for j, pwr_j in enumerate(n_pwr):
                fi, fi_var, _, w, _ = rfm.max_fi_power(pwr_j, n_units_def, n_feats)
                fi_theor_pwr[i, j] = fi[0, 0]

                rf_cents, rf_wids = rfm.get_random_uniform_fill(n_units_def, rf_distrs)
                use_wids = np.ones_like(rf_wids) * w**2
                fi, pwr, distortion = am.compute_fi(
                    rf_cents, use_wids, pwr_j, stim_distr
                )
                fi_emp_pwr[i, j] = fi
    ...
    if "de" in gen_panels:
        ...
        possibles = possibles[~np.isnan(possibles)]
        if len(possibles) > 1:
            delta = np.diff(possibles)[0]
            bounds = np.concatenate(
                (possibles - delta / 2, [possibles[-1] + delta / 2])
            )
        else:
            bounds = (possibles[0] - 0.5, possibles[0] + 0.5)
        ...
        # feat_ind = 4
        ov_ind = 0
        gpl.add_vlines(total_snrs[pwr_ind], map_ax, alpha=0.2)
        gpl.add_hlines(total_units[neurs_ind], map_ax, alpha=0.5)
        for i, (nr, mse_nr) in enumerate(mse_dist.items()):
            ...
            l = gpl.plot_trace_werr(
                total_units[tot_nu_mask],
                nr * mse_nu[tot_nu_mask],
                ax=mse_nu_ax,
                color=col,
            )
            col = l[0].get_color()
            ...
            gpl.plot_trace_werr(
                total_snrs[tot_pwr_mask],
                nr * mse_pwr[tot_pwr_mask],
                ax=mse_pwr_ax,
                color=col,
            )
...
def figure_rf_integ(basefolder=bf, gen_panels=None, data=None, resample_pair=False):
    setup()
    if gen_panels is None:
    ...
    if ("abc" in gen_panels or "d" in gen_panels) and data.get("abcde") is None:
        p = 1
    ...
    r2_ax = f.add_subplot(r2_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax)
    r1_n_ax = f.add_subplot(r1_n_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax)
    r2_n_ax = f.add_subplot(r2_n_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax)
    integ_ax = f.add_subplot(integ_gs, projection="3d")
    recon1_ax = f.add_subplot(recon1_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax)
    recon2_ax = f.add_subplot(recon2_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax)
    recon1_eg_ax = f.add_subplot(
        recon1_eg_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax
    )
    recon2_eg_ax = f.add_subplot(
        recon2_eg_gs, aspect="equal", sharex=r1_ax, sharey=r1_ax
    )

    rf_axs = np.array(
        list(
            f.add_subplot(rf_i_gs, aspect="equal")
    ...
    if "abc" in gen_panels:
        ...
        common_dist = 0.2
        unique_dist = 0.5
        dists = ((0, common_dist), (1, unique_dist), (2, unique_dist))
        n_stim = 2
        n_gen = 1000
        if data.get("eg_pair") is None or resample_pair:
            print("resampling")
            out = model.generate_input_output_pairs(
                n_gen,
                n_stim,
                ret_indiv=True,
                set_dists=dists,
        ...
        rfm.visualize_random_rf_responses(
            y_hat[corr_eg_ind, 0], model.ms_out, ax=recon1_ax
        )
        rfm.visualize_random_rf_responses(
            y_hat[ae_eg_ind, 0], model.ms_out, ax=recon2_ax
        )
        ...
        rfm.visualize_random_rf_responses(
            r1_inp[0], model.ms_f1, ax=r1_ax, vis_dims=(1, 0)
        )
        rfm.visualize_random_rf_responses(
            r2_inp[0], model.ms_f2, ax=r2_ax, vis_dims=(1, 0)
        )

        rfm.visualize_random_rf_responses(
            integ_targ[0], model.ms_integ, ax=integ_ax, vis_dims=integ_inds
        )
        integ_ax.view_init(elev=30, azim=30)

        rfm.visualize_random_rf_responses(
            recon_ops[corr_eg_ind, 0], model.ms_out, ax=recon1_eg_ax
        )
        rfm.visualize_random_rf_responses(
            recon_ops[ae_eg_ind, 1], model.ms_out, ax=recon2_eg_ax
        )
        ...
        r2_ax.set_xticks([0, 0.5, 1])
        r2_ax.set_yticks([0, 0.5, 1])
        r1_ax.set_yticks([0, 0.5, 1])

        integ_ax.set_xlabel("common feature")
        integ_ax.set_ylabel("unique feature 1")
        integ_ax.set_zlabel("unique feature 2")
        gpl.make_3d_bars(integ_ax, bar_len=0.5)
    ...
    if "d" in gen_panels:
        ...
        dists = np.sqrt(
            np.sum((mf1[:, np.newaxis] - mint[np.newaxis, :]) ** 2, axis=2).T
        )
        ...
        gpl.plot_scatter_average(
            dists, ws, n_bins=n_bins, ax=wm_ax, use_max=1, color=color
        )
    ...
    if "e" in gen_panels:
        ...
        fls = (
            ...
            ("end-to-end learning", "ff_0_3694636_2022-10-17_16:05:39.151064.pkl"),
            (
                "end-to-end learning with hidden layers",
                "ff_0_3694635_2022-10-17_16:21:01.116523.pkl",
            ),
            ("integration learning", "ff_0_3938662_2022-11-11_15:18:57.679311.pkl"),
            (
                "integration learning with hidden layers",
        ...
        fls = (
            ("end-to-end learning with no integration layer", "ff_ramp_0_4426610.pkl"),
            ("end-to-end learning", "ff_ramp_0_4375499.pkl"),
            ("end-to-end learning with hidden layer", "ff_ramp_0_4375497.pkl"),
            ("integration learning", "ff_ramp_0_4375493.pkl"),
            ("integration learning with hidden layers", "ff_ramp_0_4375495.pkl"),
        )
        ...
        for i, (label, fl) in enumerate(fls):
            ...
            color = colors[i]
            aer_ax.plot(dists, mr[0], linewidth=1, color=color, label=label)
    ...
    if "f" in gen_panels:
        ...
        f1_rd = None
        f1_code = spc.Code(inp_pwr, f1_units, dims=dims, use_ramp=f1_rd)
        ...
        for i, ni in enumerate(neur_inds):
            rf_axs[i, 0].pcolormesh(
                x_pts, y_pts, resp_dx[..., ni], cmap=rf_cmap, rasterized=True
            )
            rf_axs[i, 1].pcolormesh(
                x_pts, y_pts, resp_dy[..., ni], cmap=rf_cmap, rasterized=True
            )
            ...
            if i < len(rf_axs) - 1:
                ...
                rf_axs[i, 1].set_xlabel("azimuthal position")
...
def get_rf_resp_grid(rf, n_pts=100, bounds=(0, 1)):
    ...
    xy = np.array(list(it.product(pts, repeat=2)))
    resp_dx = rf(xy)
...
def figure6_alt(
    ...
    gen_panels=None,
    data=None,
    experiment_key="Zhang & Luck 2008",
):
    setup()
    if gen_panels is None:
    ...
    if not abort:
        ...
        axs_load = list(
            f.add_subplot(pl, sharex=ax1, sharey=ax1) for pl in pred_loads[1:]
        )
        ...
        axs_col = list(
            f.add_subplot(pl, sharex=ax1, sharey=ax1) for pl in pred_colors[1:]
        )
        axs_col = [ax1] + axs_col
        up = (f, axs_col)
        if "e" in gen_panels:
            ...
            modd_dict = da.model_subj_org(
                d1_dict, org_func=da.mse_by_dist, model=m1_dict
            )
        ...
        axs_ppc = list(f.add_subplot(pl, sharex=ax1, sharey=ax1) for pl in pred_ppc[1:])
        axs_ppc = [ax1] + axs_ppc
        up = (f, axs_ppc)
        if "f" in gen_panels:
            d1_dict, m1_dict = data["def"]
...
def figure7(basefolder=bf, gen_panels=None, data=None):
    ...
    if "ab" in gen_panels:
        ...
        out = m.random_example(
            eg_stim, make_others=True, set_dists=eg_dists, noise_mag=0, topogriphy=True
        )
    ...
    if "c" not in data.keys() and "c" in gen_panels:
        ...
        h12 = m12.train(1, validate_n=2, train_size=train_size_th, epochs=n_epochs)

        m23 = ff.IntegrationModel(*model_args, **model_kwargs)
        h23 = m23.train(2, validate_n=3, train_size=train_size_th, epochs=n_epochs)
        data["c"] = (h12, h23)
    ...
    n_stim = 2
    n_est = 10e3
    noise_mags = np.linspace(0.1, 1, 3)
    dists = np.linspace(0.2, 3, 20)
    use_dim = 0
    excl_close = 2
    boots = 1000
    if "d" not in data.keys() and "d" in gen_panels:
        ...
        for nm in noise_mags:
            dist_rates = m.estimate_ae_rate_dists(
                n_stim,
                dists,
                nm,
                n_est=n_est,
            ...
            dist_theor = am.distance_error_rate(dists, None, 0.5 * (nm**2))
    ...
    if "d" in gen_panels:
        ...
        for i, (dist_rates, dist_theor) in enumerate(pairs):
            ...
            col = l[0].get_color()
            gpl.plot_trace_werr(
                dists, dist_theor[0], ax=dist_m_ax, color=col, linestyle="dashed"
            )
    ...
    if "e" not in data.keys() and "e" in gen_panels:
        ...
        for a in asyms:
            ...
            dist_theor = am.distance_error_rate(
                dists, np.array([a]), np.array([overall_d])
            )
            pairs.append((dist_rates, dist_theor))
        data["e"] = pairs
    ...
    noise_mags = np.linspace(0.1, 1, 50)
    n_est = 10e4
    ns = (2, 3, 4)
    if "f" not in data.keys() and "f" in gen_panels:
        ...
        for i, n in enumerate(ns):
            noise_rates = m.estimate_ae_rate_noise(
                n, noise_mags, n_est=n_est, excl_close=excl_close
            )
            ds = noise_mags**2
            theor_rates = am.integrate_assignment_error((n,), ds, ds, 1, p=p)
