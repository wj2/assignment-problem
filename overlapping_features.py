...
def weighted_tradeoff(
    distorts, ae_rates, n_features, overlaps, overlap_dim=2, n_feat_dim=1
):
    weight = {
        nr: _compute_assignment_distortion(n_features, overlaps, nr)
...
def max_fi_per_feature(
    pwr_per_feature, n_units_per_feature, dims, ret_min_max=True, use_w=None, **kwargs
):
...
def post_estimated_mse(est_arr, conf_dict, pwr=None, n_units=None, dim=None):
    ...
    for ind in u.make_array_ind_iterator(est_arr.shape):
        ...
        if pwr is not None and n_units is not None:
            ...
            prob, mag = rfm.compute_threshold_err_prob(
                pwr_i, n_i, dim, w_use, resp_scale=resp_use
            )
...
def estimate_mse(pwrs, n_units, dims, div=2, **kwargs):
    ...
    out_div, c_div = rfm.emp_rf_decoding(
        pwrs / div, (n_units / div).astype(int), dims / div, **kwargs
    )
    ret_dict = {0: (out_full, c_full), div: (out_div, c_div)}
    return ret_dict


@u.arg_list_decorator
def explore_fi_tradeoff(
    n_units, total_dims, overlaps, total_pwrs, n_regions=(1, 2), **kwargs
):
...
@u.arg_list_decorator
def explore_mse_tradeoff_parallel(
    n_units, total_dims, overlaps, total_pwrs, n_regions=(1, 2), n_jobs=-1,
    nosum=False, **kwargs
):
    arr_shape = (len(n_units), len(total_dims), len(overlaps), len(total_pwrs))
...
@u.arg_list_decorator
def explore_fi_tradeoff_parallel(
    n_units, total_dims, overlaps, total_pwrs, n_regions=(1, 2), n_jobs=-1, **kwargs
):
    arr_shape = (len(n_units), len(total_dims), len(overlaps), len(total_pwrs))
    distorts = {nr: np.zeros(arr_shape) for nr in n_regions}
    ae_rates = {nr: np.zeros_like(distorts[nr]) for nr in n_regions}
...
@u.arg_list_decorator
def compute_transition(total_dims, total_pwrs, total_units, nrs=(1, 2, 3), overlap=1):
    l_mse = np.zeros((len(nrs), len(total_dims), len(total_pwrs), len(total_units)))
    ...
    for nr_i, td_i, tp_i, tu_i in u.make_array_ind_iterator(l_mse.shape):
        ...
        out = mse_tradeoff(tu, td, n_regions=(nr,), overlap=overlap, total_pwr=tp)
...
def mse_tradeoff(
    ...
    **kwargs
    ...
    for nr in n_regions:
        ...
        if np.all(np.array(dims_per_region) > overlap):
            ...
            for j, k in it.combinations(range(nr), 2):
                ae_risk = integrate_assignment_error(
                    (n_stim,),
                    ae_distorts[nr][j : j + 1],
                    ae_distorts[nr][k : k + 1],
                    overlap,
                    p=1,
                )
                ae_nr[j, k] = ae_risk
        ...
        total_err[nr] = calc_te(
            distorts[nr], ae_rates[nr], mis_prob[nr], mis_err[nr], nr, n_stim=n_stim
        )
    return total_err, distorts, ae_rates, mis_prob, mis_err


def calc_te(d_l, p_ae, p_nl, d_nl, n_regions, n_stim=2, p_thr=0.1):
    # local errors only
    # print(d_l.shape, p_ae.shape)
        ...
        if np.all(np.array(dims_per_region) > overlap):
            ...
            for i, dpr_i in enumerate(dims_per_region):
                ...
                if use_theory:
                    ...
                    distorts[nr][i] = rf_distortion(
                        ri_units, dpr_i, scale=ri_pwr, **kwargs
                    )
            ae_nr = 0
            for j, k in it.combinations(range(nr), 2):
                ae_risk = integrate_assignment_error(
...
def rf_distortion_theory(
    n_units, n_dims, input_distr_type="uniform", pwr=1, var=1, use_rand_rfs=True
):
    ...
    fi_theor = rfm.random_uniform_fi(
        n_units, np.sqrt(ws[0]), n_dims, scale=rescale, sigma_n=var
    )
...
def rf_distortion(
    ...
    if use_rand_rfs:
        ...
        ms, ws = rfm.get_output_func_distribution_shapes(
            n_units_pd, distr_list, wid_scaling=wid_scaling
        )
    rf, drf = rfm.make_gaussian_vector_rf(
        ms, ws, scale, baseline, titrate_pwr=stim_distr
    )
...
def compute_fi(ms, ws, scale, stim_distr, baseline=0, n_samps=10000, **kwargs):
    rf, drf = rfm.make_gaussian_vector_rf(
        ms, ws, scale, baseline, titrate_pwr=stim_distr
    )
    samps = stim_distr.rvs(n_samps)
...
def fixed_distance_ds_dxdy(ds, dxs, dys, n_ests=1000, p=100, c=1, boot=True):
    ...
    for i, d in enumerate(ds):
        for j, dx_j in enumerate(dxs):
            dy_j = dys[j]
            errs[i, j] = fixed_distance_errors(
                d, dx_j, dy_j, n_ests=n_ests, p=p, c=c, boot=boot
            )
    return errs
...
def estimate_ae_sr_s_ranges(esses, srs, n_pops=2, n=500, p=100, c=1, boot=True):
    ...
    for i, s in enumerate(esses):
        errs[i] = estimate_ae_sr_range(s, srs, n_pops=n_pops, n=n, p=p, c=c, boot=boot)
    return errs
...
def error_approx(p, s, pop_ests, integ_step=0.0001, eps=0):
...
def constrained_distortion_info(
    esses, distortion, p=100, diff_ds=1000, eps=0.1, d_mult=10
):
...
def minimal_error_redund(
    ...
    t1, t2, redund, pe = constrained_distortion_info(
        esses, distortion, p=p, diff_ds=diff_ds, eps=eps, d_mult=d_mult
    )
    i1, d1 = t1
    i2, d2 = t2
...
def compute_ds(b, k, c, delt, n_stim, s=100, repl_nan=True, source_distrib="gaussian"):
    if source_distrib.lower() not in ("gaussian", "uniform"):
        raise Exception("this function only works for Gaussian and Uniform " "sources")
    """ Gaussian is N(0, s^2) """
...
def _get_ae_ev(bit, k, c_xy, n_stim, s, delt, source_distrib="uniform"):
    delt = delt[0]
    ds = compute_ds(bit, k, c_xy, delt, n_stim, s=s, source_distrib=source_distrib)
    dx, dy = dxdy_from_dsdelt(ds, delt)
    dx = np.array([dx])
    dy = np.array([dy])
    aes = integrate_assignment_error((n_stim,), dx, dy, c_xy, p=s)
    return aes[0, 0], ds


def _ae_ev_loss(*args, target_ae=0.001, **kwargs):
...
def ae_ev_bits(
    bits, k, s, n_stim, c_xys, delts, source_distrib="uniform", compute_redund=False
):
    ...
    for i, c_xy in enumerate(c_xys):
        for j, delt in enumerate(delts):
            for l, bit in enumerate(bits):
                ds = compute_ds(
                    bit, k, c_xy, delt, n_stim[0], s=s, source_distrib=source_distrib
                )
                dx, dy = dxdy_from_dsdelt(ds, delt)
                dx = np.array([dx])
                dy = np.array([dy])
                aes[i, j, l] = integrate_assignment_error(n_stim, dx, dy, c_xy, p=s)
                evs[i, j, l] = ds
                redund[i, j, l] = feature_redundant_info(n_stim, dx, dy, c_xy, p=s)
    if compute_redund:
        out = aes, evs, redund
    else:
        out = aes, evs
...
def distortion_func(bits, features, objs, overlaps, d1=None, p=100, d_cand=None):
    if d1 is not None and np.all(d1 > 0):
        d = _distortion_func_numerical(
            bits, features, objs, overlaps, d1, p=p, d_cand=d_cand
        )
    else:
...
def _distortion_func_numerical(
    bits,
    features,
    objs,
    overlaps,
...
def weighted_errors_bits_feats(
    bits_list,
    features,
    objs_list,
    overlaps_list,
...
def weighted_errors_bits(
    ...
    totals = np.zeros(
        (len(bits_list), len(objs_list), len(overlaps_list), d1_size, lam_size)
    )
    assignment_errs = np.zeros(
        (len(bits_list), len(objs_list), len(overlaps_list), d1_size)
    )
...
def weighted_errors_lambda(
    ...
    for i, overlaps in enumerate(overlaps_list):
        ...
        if d1_n == 1 and np.all(delta_d == 0):
            overall_d = distortion_func(bits, features, objs, overlaps, d1=None, p=p)
        else:
            overall_d = distortion_func(
                bits, features, objs, overlaps, d1=d1_i, p=p, d_cand=initial_d
            )
        assignment_d = integrate_assignment_error(obj_list, d1_i, d2_i, overlaps, p=p)
        local_d[i] = overall_d
...
def minimum_weighted_error_lambda(
    bits,
    features,
    objs,
    p=100,
    ...
    if lam_range is None:
        lam_range = np.linspace(lam_beg, lam_end_mult * p * features * objs, lam_n)
    out = weighted_errors_lambda(
        bits, features, objs, overlaps, d1_list=d1_list, lam_range=lam_range, p=p
    )
    total, local_d, ae, d1s = out
    flat_shape = (len(overlaps) * len(d1_list), len(lam_range))
    o_i, d1_i = np.unravel_index(
        np.argmin(total.reshape(flat_shape), axis=0), (len(overlaps), len(d1_list))
    )
    opt_d1 = d1_list[d1_i]
