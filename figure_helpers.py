...
def plot_eg_overlap(
    ax1, ax2, n_stim=2, eg_s=100, color1=(0.6, 0.6, 0.6), color2=(0.6, 0.6, 0.6)
):
...
def plot_noise_eg(
    ...
    for i, sl in enumerate(stim_locs):
        ...
        x_ax.text(
            sl + noise_delts[0, i], 0 - txt_offset, label_x, ha="center", va="top"
        )
        y_ax.text(
            sl + noise_delts[1, i], 1 + txt_offset, label_y, ha="center", va="bottom"
        )
...
def plot_distance_error_prob(
    ax,
    distortions=(1, 10, 20),
    color=None,
    distance_bounds=(0.1, 20),
    n_dists=1000,
    label=True,
    label_num=None,
    line_label=False,
    ...
    for i, distort in enumerate(distortions):
        ...
        gpl.plot_trace_werr(
            distances, swap_prob, ax=ax, label=legend_label, color=color
        )
        if line_label:
            ms = np.array(
                [
...
def plot_distance_distrib_prob(
    ax, overlap_dims=(1,), funcs=(am.line_picking_line,), n_dists=1000
):
...
def plot_ae_rate(a_rate, a_approx, pr, s_counts, ax, colors=None):
    for i, s in enumerate(s_counts):
        ...
        gpl.plot_trace_werr(pr, a_approx[i], ax=ax, linestyle="dashed", color=colors[i],
                            plot_outline=True)
...
def plot_stan_model(model, ae_ax, dist_ax, uni_ax, n=4, spacing=np.pi / 4, sz=8):
    ...
    gpl.plot_trace_werr(
        x_pos, ae_prob_arr, points=True, ax=ae_ax, error_func=gpl.conf95_interval
    )

    dist_arr = np.expand_dims(dist, 1)
    p = dist_ax.violinplot(dist, positions=x_pos, showextrema=False)
    gpl.plot_trace_werr(
        x_pos, dist_arr, points=True, ax=dist_ax, error_func=gpl.conf95_interval
    )

    up_arr = np.expand_dims(unif_prob, 1)
    p = uni_ax.violinplot(up_arr, positions=x_pos, showextrema=False)
    gpl.plot_trace_werr(
        x_pos, up_arr, points=True, ax=uni_ax, error_func=gpl.conf95_interval
    )
    ...
    gpl.make_yaxis_scale_bar(
        uni_ax, anchor=0, magnitude=0.2, double=False, label="guess rate", text_buff=0.7
    )


def plot_multidim_schem(p1, p2, ax, non_alpha=0.5):
    p1 = np.array(p1)
    p2 = np.array(p2)
    sp = np.stack((p1, p2), 0)
    ax.grid(False)
...
def plot_delta_xy(
    ds, circ_pt_delts, ax, color_st=None, line_color=(0.8, 0.8, 0.8), color_incr=0.1
):
    delta_eg = np.linspace(-0.9, 0.9, 1000)
...
def estimate_empirical_aes(
    dims, stim_count, pr, size, delta_d=(0,), n_ests=5000, boot=True
):
    ds = (size / pr) ** 2
    ...
    for i, od in enumerate(dims):
        for j, dd in enumerate(delta_d):
            for k, ds_i in enumerate(ds):
                out = am.estimate_ae_full(size, stim_count, dd, ds_i, od, n_ests=n_ests)
                if boot:
                    out = u.bootstrap_on_axis(out[0], u.mean_axis0, axis=0, n=n_ests)
                aes[i, j, k] = out
    return aes
...
def plot_ae_error(
    ax1, ax2, aes, redund, ols, pr, cut_end=10, delta_d=None, aes_est=None, col_mult=0.1
):
    ...
    if delta_d is None:
        for i, ae in enumerate(aes):
            if aes_est is not None:
                theor_kwargs = {'plot_outline': True, 'linestyle': 'dashed'}
                emp_kwargs = {'label': "C = {}".format(ols[i])}
            else:
                theor_kwargs = {'label': "C = {}".format(ols[i])}
                emp_kwargs = {}
            ...
            gpl.plot_trace_werr(
                pr[:-cut_end], redund[i, :-cut_end], ax=ax2, log_x=True, log_y=False
            )
            if aes_est is not None:
...
def plot_asymmetric_eg(ds, delt, integ_ax, indiv_ax, colors, n_pts=1000, sd_mult=3):
    ...
    gpl.plot_trace_werr(
        pdf_pts, g_s, ax=integ_ax, color=c_s, label=r"$s | \hat{x}, \hat{y}$"
    )
    gpl.plot_trace_werr(pdf_pts, g_x, ax=indiv_ax, color=c_x, label=r"$s | \hat{y}$")
    gpl.plot_trace_werr(pdf_pts, g_y, ax=indiv_ax, color=c_y, label=r"$s | \hat{x}$")
...
def plot_rdb(ax, bits_range=(0, 20), n_bits=1000, size=100):
    ...
    imp_region = plt.Polygon(poly_pts, color="r", alpha=0.2)
