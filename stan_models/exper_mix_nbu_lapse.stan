...
//sizes 
  ...
  real<lower=0> lr_beta_k;
  real<lower=0> lr_beta_t;
  real<lower=0> lr_alpha_k;
  real<lower=0> lr_alpha_t;
...
parameters {
  ...
  real<lower=0> lapse_rate_alpha;
  real<lower=0> lapse_rate_beta;

  // data-related
  vector<lower=-report_mse1_mean/report_mse1_var>[S] report_mse1_raw;
  vector<lower=-dist_mse1_mean/dist_mse1_var>[S] dist_mse1_raw;
  vector<lower=-mech_mse_mean/mech_mse_var>[S] mech_mse_raw;
  vector<lower=-encoding_rate_mean/encoding_rate_var>[S] encoding_rate_raw;
  vector<lower=0, upper=1>[S] lapse_rate;
}
...
model {
  ...
  lapse_rate_alpha ~ gamma(lr_alpha_k, lr_alpha_t);
  lapse_rate_beta ~ gamma(lr_beta_k, lr_beta_t);
  
  report_mse1_raw ~ normal(0, 1);
  dist_mse1_raw ~ normal(0, 1);
  mech_mse_raw ~ normal(0, 1);
  encoding_rate_raw ~ normal(0, 1);
  lapse_rate ~ beta(lapse_rate_alpha, lapse_rate_beta);
  ...
  for (t in 1:T) {
	...
	enc_lprob = ne_prob;
      ...
      enc_lprob = log(1 - lapse_rate[subj]) + enc_lprob;
...
generated quantities {
  ...
  for (t in 1:T) {
    ...
    if (ec == 0 || bernoulli_rng(lapse_rate[subj]) == 1) {
